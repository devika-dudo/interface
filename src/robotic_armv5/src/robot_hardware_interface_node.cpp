#include <robotic_armv5/robot_hardware_interface.h>
...
ROBOTHardwareInterface::ROBOTHardwareInterface(ros::NodeHandle& nh) : nh_(nh) {
	...
	client = nh_.serviceClient<robotic_armv5::Floats_array>("/read_joint_state");
...
void ROBOTHardwareInterface::read() {
	...
	if(client.call(joint_read))
	{
		...
		joint_position_[5]=angles::from_degrees(joint_read.response.res[5]-90);
...
void ROBOTHardwareInterface::write(ros::Duration elapsed_time) {
	...
	//ROS_INFO("Publishing j1: %.2f, j2: %.2f, j3: %.2f, j4: %.2f, j5: %.2f, j6: %.2f",joints_pub.data[0],joints_pub.data[1],joints_pub.data[2],joints_pub.data[3],joints_pub.data[4],joints_pub.data[5]);
